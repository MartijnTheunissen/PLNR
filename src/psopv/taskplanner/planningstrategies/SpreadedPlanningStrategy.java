 ...
 */
public class SpreadedPlanningStrategy implements PlanningStrategy {

	private final static Logger	log				= LogManager.getLogger(SpreadedPlanningStrategy.class);
	...
	public Calendar plan(ArrayList<Calendar> calendars, TaskList tasklist, String calendarName) throws PlanningException {
		...
		for (int i = 0; i < tasklist.getSize(); i++) {
			...
			ArrayList<FreeTimeContainer> filteredCopy = (ArrayList<FreeTimeContainer>) freeTime.stream().filter(deadlineOK)

			.sorted((l1, l2) -> {
				return l1.getDurationLeft().compareTo(l2.getDurationLeft());
			})
	...
	private FreeTimeContainer getBestContainer(Task toPlan, ArrayList<FreeTimeContainer> possibilities) throws PlanningException {
		...
		if (possibilities != null && possibilities.size() > 0) {
			...
			longestDuration = possibilities.get(index).getDurationLeft().minus(toPlan.getDuration());
		...
		for (FreeTimeContainer container : possibilities) {
			Duration newDuration = container.getDurationLeft().minus(toPlan.getDuration());
			if (!newDuration.isNegative() && newDuration.compareTo(longestDuration) > 0) // found a better match
			{
	...
	public String toString() {
		return ApplicationSettings.getInstance().getLocalizedMessage("plan.spread");
	}
}
